...
public class BookingServiceTestsPositive
{
    ...
    public async Task CreateAsync_ValidOneBooking_ReturnsPendingBooking()
    {
        ...
        var bookingService = _dbContextMocker.ArrangeBookingServiceTestCase(dbContext, null);
    ...
    [Fact]
    public async Task ProcessPendingBookingAsync_GetInProcessStatus_ReturnsBooking()
    {
        Guid eventId = Guid.NewGuid();

        var dbContext = _dbContextMocker.GetAppDbContext(nameof(this.ProcessPendingBookingAsync_GetInProcessStatus_ReturnsBooking));
        var bookingService = _dbContextMocker.ArrangeBookingServiceTestCase(dbContext, null);
        ...
        result.Id.Should().Be(bookingInfo.Id);
        result.EventId.Should().Be(bookingInfo.EventId);
        result.Status.Should().Be(BookingStatusEnum.InProcessing);
        result.CreatedAt.Should().Be(bookingInfo.CreatedAt);
