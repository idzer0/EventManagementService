...
namespace EventMS.Libs.KafkaShared;
...
public abstract class KafkaConsumerService : BackgroundService
{
    ...
    protected abstract Task HandleMessageAsync(string key, string value, IServiceProvider scopeServiceProvider);
        ...
        try
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    await HandleMessageAsync(consumeResult.Message.Key, consumeResult.Message.Value, scope.ServiceProvider);
